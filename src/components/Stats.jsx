...
export default function Stats() {
  ...
  const items = [
    {
      icon: <Users size={52} color="green" />,
      title: "Total Users",
      value: stats.total,
    },
    {
      icon: <UserCheck size={52} color="green" />,
      title: "Active Users",
      value: 100 - (+stats.blockedPercentage + +stats.inactivePercentage),
    },
    {
      icon: <UserX size={52} color="green" />,
  ...
  return (
    <div className="grid grid-cols-4 gap-4 p-4 max-lg:grid-cols-2 max-sm:grid-cols-1">
      {items.map((stat, index) => (
        <div
          key={index}
          className="flex items-center  bg-white shadow-md rounded-xl px-4 py-10  border-[1px] border-gray-300"
        >
          ...
          <div className="ml-4">
            ...
            <p className="text-4xl font-bold mt-2  text-gray-700">
              {stat.value + (index >= 2 && "%")}
            </p>
          </div>
        </div>
